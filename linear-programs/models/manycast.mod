...
subject to groupCausesConnectionToFailIncludeEndpoints_1{d in D, i in I, g in GroupIndices}:
    FG_Conn_include_endpoints[d,i,g] <= sum{u in V, v in V: u != v and ((u,v) in FG[g] or (v,u) in FG[g])} L[s,d,i,u,v] + sum{v in V: (v,v) in FG[g] and v != s} NC[d,i,v];

subject to groupCausesConnectionToFailIncludeEndpoints_2{d in D, i in I, g in GroupIndices}:
    FG_Conn_include_endpoints[d,i,g] * card(V)^4 >= sum{u in V, v in V: u != v and ((u,v) in FG[g] or (v,u) in FG[g])} L[s,d,i,u,v] + sum{v in V: (v,v) in FG[g] and v != s} NC[d,i,v];
...
subject to fAtMostOncePair{d in D, u in V, v in V, g in GroupIndices: u != v and ((u,v) in FG[g] or (v,u) in FG[g])}:
    sum{i in I} L[s,d,i,u,v] <= 1;

subject to fAtMostOncePair_nodes{d in D, v in V, g in GroupIndices: (v,v) in FG[g] and v != s and v != d}:
    sum{i in I} NC[d,i,v] <= 1;
